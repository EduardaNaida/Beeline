...
/**
 *  Displays information for the in line page for the outside queue for a chosen store. Like the logo, 
...
function InLinePage(){
    ...
    useEffect(() => {
        window.intervalStore1 = setInterval(getSensor, 20000);//20sec
        window.intervalStore2 = setInterval(getPlace, 20000);//20sec
        window.intervalStore3 = setInterval(getWait, 40000);//20sec
                ...
                if (store.ID === id){
                    ...
                    for(let x of window.activeQueues){
                        if(x[0].Name === store.Name){
                            queueToken = x[1];
                            axios.get('/API/v1/queue/'+id, {headers: {'Queue-Token': queueToken }})
                            .then((response) => {
        ...
        /************* get  waitingtime *************/
        axios.get('/API/v1/queue/waittime/'+id)
        .then((response) => {
            if ((response.data.WaitTime)/60 < 1){
                setWaitTimeofQueue("<1");
            }
            else{
                setWaitTimeofQueue(Math.round((response.data.WaitTime)/60));
            }  
        })
    ...
    function getPlace(){
        ...
        if((response.data.Index+1) <= 1){
           return history.push(`/yourturn/${id}`);
        } else{
            return;
        }
        })
    ...
    return(
        ...
        {getPlace()}
            ...
            <Card className={classes.card}>
                ...
                <div className={classes.width} style={{height: 'fit-content'}}>
