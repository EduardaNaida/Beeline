...
class CaptchaTest extends Component {
 ...
 constructor(props){ 
   ...
   doSubmit = () => {
       ...
       if (validateCaptcha(user_captcha) === true) {
            /************* post to queue *************/
            axios.post('/API/v1/queue/'+this.props.store.ID)
            .then((response) => {
                const token= response.data.QueueToken;
                window.activeQueues.push([this.props.store, token, false])
 ...
 //state1: { token: this.state.queuetoken }
   render() {

        const { classes } = this.props;

        const path = `/queue/${this.id}`;
...
export default withStyles(styles, { withTheme: false })(CaptchaTest);
